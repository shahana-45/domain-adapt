...
class Config(object):
    ...
    def __init__(self, dataset, cuda=0, finetune=True, bertmodel='roberta', lambda4=1.0, unlabeled_dataset=None):
        # print(dataset)
        ...
        if unlabeled_dataset:
            self.annotation_path = unlabeled_dataset + '/data/train.txt'
        else:
        ...
        print(self.log)
        self.device = torch.device('cuda:{' + str(cuda) + '}'.format(cuda) if torch.cuda.is_available() else 'cpu')  #
        ...
        if self.bert == 'roberta':
            self.batch_size = 32
            self.bert_path = './pretrained/roberta-base'  # change . to .. or vice-versa
            print(os.getcwd())
            self.hidden_size = 768
        self.tokenizer = RobertaTokenizer.from_pretrained(self.bert_path, use_auth_token=True)
        ...
        self.label_num = 211  # 117    # total label num(top:4,second:11,conn:102)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=int, default=1, choices=[1, 2], help='train dataset: 1 for PDTB3, 2 for '
                                                                               'DiscoGem')
