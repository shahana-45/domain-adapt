...
def build_dataset(config, test=False, semi_supervision=False, arg1=None, arg2=None, label1=None, label2=None, is_golf_data=False):
    def load_pseudo_labeled_data(arg1, arg2, label_top, label_sec, pad_size=100):
        ...
        return (input_ids, seq_len, attention_mask, token_type_ids,
                                         label_top, label_sec, 1,
                                         -1, -1, 1, arg1_mask, arg2_mask)

    def load_dataset(path, pad_size=100, semi_supervised=False):
        contents = []
        with open(path, 'r', encoding='UTF-8', errors="replace") as f:
            ...
            for line in tqdm(f):
                ...
                if not lin:
                    continue
                #print(lin.split('|||')) # TODO remove print
                ...
                if test:
                    try:
                        ...
                        if labels1[2] in config.conn2i.keys():
                            labels1[2] = config.conn2i[labels1[2]]
                        else:
                            labels1[2] = 1
                        ...
                        if labels2[2] in config.conn2i.keys():
                            labels2[2] = config.conn2i[labels2[2]]
                        else:
                            labels2[2] = 1
                ...
                if semi_supervised:
                    contents.append((input_ids, seq_len, attention_mask, token_type_ids,
                                 ...
                                 arg1_mask, arg2_mask, {"arg1": arg1, "arg2": arg2}))
                else:
                    contents.append((input_ids, seq_len, attention_mask, token_type_ids,
                                     labels1[0], labels1[1], labels1[2],
                                     labels2[0], labels2[1], labels2[2],
        ...
        return contents
    if arg1 and arg2:
        return load_pseudo_labeled_data(arg1, arg2, label1, label2)
    train = load_dataset(config.train_path, config.pad_size)
    dev = load_dataset(config.dev_path, config.pad_size)
    test = load_dataset(config.test_path, config.pad_size)
    if semi_supervision:
        unlabeled_annotations = load_dataset(config.annotation_path, config.pad_size, semi_supervised=True)
        return train, dev, test, unlabeled_annotations
    else:
