...
from ldsgm_utils import get_time_dif
...
def test(config, model, test_iter, reverse=False, ensemble=False, only_eval=False):
    if only_eval:
        model.load_state_dict(torch.load(config.save_path_top, map_location=torch.device('cpu')),  strict=False)
    else:
        model.load_state_dict(torch.load(config.save_path_top),  strict=False)
    model.eval()
    start_time = time.time()
...
def evaluate(config, model, data_iter, test=False, reverse=False, ensemble=False, semi_supervision=False):
    model.eval()
    loss_total = 0
    ...
    with torch.no_grad():
        for texts, y1, y2, argmask in data_iter:
            outputs_top, outputs_sec, outputs_conn, outputs_top_reverse, outputs_sec_reverse, outputs_conn_reverse = model(
                texts, argmask)
            if reverse:
                outputs_top = outputs_top_reverse  # TODO understand dimensions
            ...
            # print(outputs_top)
            # print(y1[0])
            loss_top = F.cross_entropy(outputs_top, y1[0])  # TODO should we rank softmax or cross-entropy for DA?
            loss_sec = F.cross_entropy(outputs_sec, y1[1])
            # print(y1[2])
            loss_conn = F.cross_entropy(outputs_conn, y1[2])
    ...
    if test:
        ...
        report_sec = metrics.classification_report(gold_sense_sec, predict_sense_sec,
                                                   digits=4)
        confusion_sec = metrics.confusion_matrix(gold_sense_sec, predict_sense_sec)
        labels_top = ['Temporal', 'Contingency', 'Comparison', 'Expansion']
        labels_top = ['Temporal', 'Contingency', 'Comparison', 'Expansion']
        # plot top level confusion matrix
        labels = ['Temporal.Asynchronous', #'Temporal.Synchronous',
                  'Contingency.Cause',
                   #'Contingency.Cause+Belief', 'Contingency.Condition',
                  'Contingency.Purpose',
                  'Comparison.Contrast', 'Comparison.Concession', 'Expansion.Conjunction', 'Expansion.Equivalence',
                  'Expansion.Instantiation', 'Expansion.Level-of-detail', #'Expansion.Manner',
                   'Expansion.Substitution']
        #ConfusionMatrixDisplay.from_predictions(gold_sense_top, predict_sense_top, display_labels=labels_top)

        # plot second level confusion matrix
       # ConfusionMatrixDisplay.from_predictions(gold_sense_sec, predict_sense_sec, display_labels=labels)
       # plt.show()
        if semi_supervision:
            return loss_total / len(
                data_iter), acc_top, f1_top, report_top, confusion_top, acc_sec, f1_sec, report_sec, confusion_sec, acc_conn, f1_conn, predict_sense_top
        return loss_total / len(
            data_iter), acc_top, f1_top, report_top, confusion_top, acc_sec, f1_sec, report_sec, confusion_sec, acc_conn, f1_conn
    if semi_supervision:
        return loss_total / len(data_iter), acc_top, f1_top, acc_sec, f1_sec, acc_conn, f1_conn, predict_sense_top
    return loss_total / len(data_iter), acc_top, f1_top, acc_sec, f1_sec, acc_conn, f1_conn


# this method is for domain adaptation (returns public annotation predictions)
def evaluate_without_gold_data(config, model, data_iter, reverse=False, ensemble=False):
    model.eval()
    predict_all_top = np.array([], dtype=int)
    predict_all_sec = np.array([], dtype=int)
    confidence_top = np.array([], dtype=float)
    confidence_sec = np.array([], dtype=float)
    ...
    with torch.no_grad():
        for texts, y1, y2, argmask in data_iter:
            ...
            softmax_top = F.softmax(outputs_top.clone().detach())
            # move tensor to cpu as tensor on gpy cannot be converted to numpy array
            confidence_top = np.append(confidence_top, torch.max(softmax_top.cpu(), 1)[0])
            # print(confidence_top_1)
            softmax_sec = F.softmax(outputs_sec.clone().detach())
            confidence_sec = np.append(confidence_sec, torch.max(softmax_sec.cpu(), 1)[0])
            # print(confidence_sec_1)
            if config.need_clc_loss:
                # print(outputs_top.data.size())
                y_predit_top = torch.max(outputs_top.data, 1)[1].cpu().numpy()
                y_predit_sec = torch.max(outputs_sec.data, 1)[1].cpu().numpy()
    ...
    return predict_all_top, predict_all_sec, confidence_top, confidence_sec


def evaluate_without_gold_data_golf(model, data_loader):
    model.eval()
    loss_total = 0
    predict_all_top = np.array([], dtype=int)
    predict_all_sec = np.array([], dtype=int)
    ...
    with torch.no_grad():
        for i, ((x, _, mask, _), (y1_top, y1_sec, y1_conn), (y2_top, y2_sec, y2_conn), (arg1_mask, arg2_mask)) in enumerate(
                data_loader):
            logits_top, logits_sec, logits_conn = model(x, mask, y1_top, y1_sec, y1_conn, arg1_mask, arg2_mask,
                                                        train=False)

            softmax_top = F.softmax(logits_top.clone().detach())
            # move tensor to cpu as tensor on gpu cannot be converted to numpy array
            confidence_top = np.append(confidence_top, torch.max(softmax_top.cpu(), 1)[0])
            # print(confidence_top_1)
            softmax_sec = F.softmax(logits_sec.clone().detach())
            confidence_sec = np.append(confidence_sec, torch.max(softmax_sec.cpu(), 1)[0])
