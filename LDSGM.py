...
class Model(nn.Module):
    def __init__(self, config):
        ...
        with open('./label_graph.g', 'rb') as f:  # change .. to .
            label_graph = pickle.load(f)
            b = numpy.matrix(label_graph)
            # numpy.savetxt('labelgraph.txt', b, fmt='%d')
        self.config = config
        self.label_graph = torch.from_numpy(label_graph).float().to(config.device)
        self.label_embedding = nn.Parameter(
            torch.randn(config.label_num, config.label_embedding_size, dtype=torch.float32))
        ...
        self.decoder = Decoder(config.label_embedding_size, config.attn_hidden_size, config.enc_hidden_size,
                               config.dec_hidden_size)
        # bottom-up manner auxilary Decoder
        self.decoder_reverse = Decoder(config.label_embedding_size, config.attn_hidden_size, config.enc_hidden_size,
                                       config.dec_hidden_size)
    ...
    def forward(self, x, argmask):
        ...
        # input_reverse = label_repr[15: 117].sum(dim=0, keepdim=True).repeat(bs, 1) # TODO
        input_reverse = label_repr[18: 211].sum(dim=0, keepdim=True).repeat(bs, 1)
        ...
        for i in range(3):
            ...
            if i == 1:  # second-level prediction
                ...
                # label2context_out = self.label2context(encode_out, label_repr, [4, 15]) # TODO uncomment
                label2context_out = self.label2context(encode_out, label_repr, [4, 18])
                pred, output = self.decoder(input, hidden.contiguous(), label2context_out)
                hidden = output
                out_sec = self.fc1_sec(pred)
                t = self.softmax(out_sec).unsqueeze(-1).expand(out_sec.shape[0], out_sec.shape[1], label_repr.shape[1])
                # input = (t * label_repr[4:15]).sum(dim=1, keepdim=False) # TODO uncomment
                input = (t * label_repr[4:18]).sum(dim=1, keepdim=False)

            if i == 2:  # conn-level prediction
                hidden = hidden + hidden_g
                # label2context_out = self.label2context(encode_out, label_repr, [15, 117]) # TODO
                label2context_out = self.label2context(encode_out, label_repr, [18, 211])
                pred, output = self.decoder(input, hidden.contiguous(), label2context_out)
                out_conn = self.fc1_conn(pred)
                t = self.softmax(out_conn).unsqueeze(-1).expand(out_conn.shape[0], out_conn.shape[1],
                                                                label_repr.shape[1])
                # input = (t * label_repr[15:117]).sum(dim=1, keepdim=False) # TODO
                input = (t * label_repr[18:211]).sum(dim=1, keepdim=False)
        ...
        for i in range(3):
            if i == 0:
                ...
                t = self.softmax(out_conn_reverse).unsqueeze(-1).expand(out_conn_reverse.shape[0],
                                                                        out_conn_reverse.shape[1], label_repr.shape[1])
                # input_reverse = (t * label_repr[15:117]).sum(dim=1, keepdim=False) # TODO
                input_reverse = (t * label_repr[18:211]).sum(dim=1, keepdim=False)

            if i == 1:
                hidden_reverse = hidden_reverse + hidden_g
                hidden_reverse = hidden_reverse.contiguous()
                # label2context_out = self.label2context(encode_out, label_repr, [4, 15]) # TODO
                label2context_out = self.label2context(encode_out, label_repr, [4, 18])
                pred_reverse, output_reverse = self.decoder_reverse(input_reverse, hidden_reverse, label2context_out)
                hidden_reverse = output_reverse
                ...
                # input_reverse = (t * label_repr[4:15]).sum(dim=1, keepdim=False) # TODO
                input_reverse = (t * label_repr[4:18]).sum(dim=1, keepdim=False)
