...
def train(
    ...
    # PDTB3 OR PDTB3+DG training set
    labeled_dataset = []  # we need different datasets for the models because they have slightly different data
    # representation
    # train dataset for LDSGM
    labeled_dataset.append(build_iterator(labeled_data[0], idrr_model_list[0].config))
    # train dataset for GOLF
    labeled_dataset.append(DataLoader(dataset=labeled_data[1],
                               batch_size=da_config.batch_size,
                               shuffle=True))
    print(len(labeled_dataset[0]))
    print(len(labeled_dataset[1]))

    # Get optimizers and schedulers
    n_labeled = [len(labeled_data[0]), len(labeled_data[1])]
    max_epoch = da_config.max_epoch

    batch_size = da_config.batch_size
    total_update_steps = [None, None]
    if da_config.bootstrapping_type in ["st", "ct"]:
        ...
        total_update_steps[0] = (n_labeled[0] + da_config.unlabeled_data_sampling_size) * max_epoch // batch_size
        warmup_steps = 7000
    else:
        raise Exception("Never occur.")

    utils.writelog("*********************Training*********************")
    utils.writelog("n_labeled for LDSGM: %d" % n_labeled[0])
    utils.writelog("n_labeled for GOLF: %d" % n_labeled[1])
    utils.writelog("max_epoch: %d" % max_epoch)
    utils.writelog("batch_size: %d" % batch_size)
    utils.writelog("total_update_steps for LDSGM: %d" % total_update_steps[0])
        ...
        for p_i in range(n_models):
            ...
            if p_i == 0:  # LDSGM
                loss_test, acc_top_test, f1_top_test, acc_sec_test, f1_sec_test, acc_conn_test, f1_conn_test \
                    = get_dev_scores("LDSGM", dev_iter[p_i], idrr_model_list[p_i], da_config,
                                     add_model_id_to_path(path_dev_pred, p_i))
            elif p_i == 1:  # GOLF
                loss_test, acc_top_test, f1_top_test, acc_sec_test, f1_sec_test, acc_conn_test, f1_conn_test \
                    = get_dev_scores("GOLF", dev_iter[p_i], idrr_model_list[p_i], golf_args,
                                     add_model_id_to_path(path_dev_pred, p_i))
            ...
            torch.save(idrr_model_list[p_i].state_dict(), add_model_id_to_path(path_snapshot, p_i)) # 0 is for LDSGM; 1 is for GOLF
    ...
    ##################
    y_ldsgm = import_module("LDSGM")  # new model for self-training
    y_golf = import_module("GOLF")  # new model for self-training
    # new LDSGM model
    semisupervised_model = [None, None]
    semisupervised_model[0] = y_ldsgm.Model(ldsgm_config).to(
        ldsgm_config.device)  # model to train using semi-supervision
    # new GOLF model
    semisupervised_model[1] = y_golf.Model(golf_args).to(ldsgm_config.device)  # model to train using semi-supervision
    ...
    param_optimizer = [None, None]
    param_optimizer[0] = list(semisupervised_model[0].named_parameters())
    param_optimizer[1] = list(semisupervised_model[1].named_parameters())
    optimizer = [None, None]
    optimizer_grouped_parameters = [None, None]
    no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight']
    optimizer_grouped_parameters_rev = [
        {'params': [p for n, p in param_optimizer[0] if any(rd in n for rd in ['decoder_reverse'])]}]
    optimizer_grouped_parameters[0] = [
        {'params': [p for n, p in param_optimizer[0] if not any(nd in n for nd in no_decay)],
         'weight_decay': 0.01},
        {'params': [p for n, p in param_optimizer[0] if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}]
    optimizer_grouped_parameters[1] = [
        {'params': [p for n, p in param_optimizer[1] if not any(nd in n for nd in no_decay)],
         'weight_decay': 0.01},
        {'params': [p for n, p in param_optimizer[1] if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}]

    # LDSGM optimizer
    optimizer[0] = BertAdam(optimizer_grouped_parameters[0],
                            lr=ldsgm_config.learning_rate,
                            warmup=0.05,
                            t_total=total_update_steps[0])
    ...
    optimizer_reverse = BertAdam(optimizer_grouped_parameters_rev,
                                 ...
                                 t_total=total_update_steps[0])
    new_model = [None, None]
    new_model[0] = idrr_model_list[0]  # initially annotate with source-only LDSGM model
    new_model[1] = idrr_model_list[1]  # initially annotate with source-only GOLF model
    seed = 42
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        if da_config.bootstrapping_type == "so":
            ...
            train_dataset_list[0] = [labeled_dataset[0]]
            train_dataset_list[1] = [labeled_dataset[1]]

        elif (epoch == 1) or ((epoch - 1) % da_config.annotation_reflesh_frequency == 0):
            ldsgm_params["optimizer"] = optimizer[0]
            ...
            # unlabeled data for LDSGM and GOLF
            unlabeled_iter = build_iterator(np.array(unlabeled_dataset)[subset_indices], new_model[0].config)
            # unlabeled_iter[1] = build_iterator(np.array(unlabeled_dataset)[subset_indices], ldsgm_config)
            ...
            with torch.no_grad():
                ann_list_top = [None, None]
                ann_list_sec = [None, None]
                confidence_scores = [None, None]
                for p_i in range(n_models):

                    if p_i == 0:
                        utils.writelog("2. Annotating for LDSGM ...")
                        ann_list_top[p_i], ann_list_sec[p_i], _, confidence_scores[p_i] = annotate("LDSGM", unlabeled_iter, new_model[p_i],
                                                                                da_config)
                    elif p_i == 1:
                        utils.writelog("2. Annotating for GOLF ...")
                        ann_list_top[p_i], ann_list_sec[p_i], _, confidence_scores[p_i] = annotate("GOLF", unlabeled_iter, new_model[p_i],
                                                                                da_config)
            ...
            for p_i in range(n_models):
                ...
                utils.writelog(
                    "[Epoch %d; Model %d] Number of pseudo-labeled data: %d (Utility: %.02f%%; Range: [%.02f, %.02f])" % \
                ...
                if da_config.bootstrapping_type == "at" and p_i == 0:
                    ...
                    if p_i == 1:
                        train_dataset = np.concatenate(
                            [np.array(labeled_data[p_i].content)[:, :12], np.array(pseudo_labeled_data)],
                            axis=0)  # combine labeled and unlabeled data (PDTB3 + genre-specific unlabeled data)
                    elif p_i == 0:
                        train_dataset = np.concatenate(
                            [np.array(labeled_data[p_i])[:, :12], np.array(pseudo_labeled_data)],
                            axis=0)  # combine labeled and unlabeled data (PDTB3 + genre-specific unlabeled data)
        ...
        # IMPORTANT!!!!
        # LDSGM (p_i = 0) will be trained with GOLF annotated unlabeled data -> model.0 is GOLF->LDSGM
        # Hence GOLF is teacher, LDSGM is student
        # GOLF (p_i = 1) will be trained with LDSGM annotated unlabeled data -> model.1 is LDSGM->GOLF
        # train LDSGM model
        ...
        for p_i in range(n_models):
            ...
            if p_i == 0:  # LDSGM
                ...
                train_ldsgm(ldsgm_config, semisupervised_model[p_i], new_train_iter, dev_iter[p_i], test_iter[p_i],
                            ldsgm_params)
                ...
                new_train_iter = DataLoader(dataset=train_dataset,
                                            ...
                                            shuffle=True)
                if optimizer[1] is None:
                    optimizer[1] = BertAdam(optimizer_grouped_parameters[1],
                                            lr=golf_args.lr,
                ...
                train_golf(golf_args, semisupervised_model[p_i], new_train_iter, dev_iter[p_i], test_iter[p_i],
                           optimizer[p_i])
        ...
        with torch.no_grad():
            for p_i in range(n_models):
                ...
                if p_i == 0:
                    loss_test, acc_top_test, f1_top_test, acc_sec_test, f1_sec_test, acc_conn_test, f1_conn_test \
                        = get_dev_scores("LDSGM", dev_iter[0], semisupervised_model[0], da_config,
                                         add_model_id_to_path(path_dev_pred, p_i))
                elif p_i == 1:
                    loss_test, acc_top_test, f1_top_test, acc_sec_test, f1_sec_test, acc_conn_test, f1_conn_test \
                        = get_dev_scores("GOLF", dev_iter[1], semisupervised_model[1], golf_args,
                                         add_model_id_to_path(path_dev_pred, p_i))
                ...
                new_model[p_i] = semisupervised_model[p_i]
                # Save the model?
                if did_update:
                    torch.save(semisupervised_model[p_i].state_dict(), add_model_id_to_path(path_snapshot, p_i))
        ...
        for student_i in range(2):
            ...
            if indicators_list[teacher_i][data_i]:
                ...
                pseudo_labeled_data = build_dataset(ldsgm_model.config, arg1=data[12]["arg1"], arg2=data[12]["arg2"],
                                                    label1=ann_list_top[teacher_i][data_i], label2=ann_list_sec[teacher_i][data_i])
                class_4_prediction_dict[class4[ann_list_top[teacher_i][data_i]]] += 1
                class_11_prediction_dict[class11[ann_list_sec[teacher_i][data_i]]] += 1
                # print(pseudo_labeled_data)
...
def train_golf(golf_args, model, train_loader, dev_loader, test_loader, optimizer):
    total_batch = 0
    dev_best_acc_top = 0.0
    dev_best_acc_sec = 0.0
    dev_best_acc_conn = 0.0
    ...
    for i, (x, _, mask, _, y1_top, y1_sec, y1_conn, _, _, _, arg1_mask, arg2_mask) in enumerate(train_loader):
        model.train()
        # print(y1_sec)
        logits_top, logits_sec, logits_conn, loss = model(x, mask, y1_top, y1_sec, y1_conn, arg1_mask, arg2_mask,
                                                          train=True)
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        ...
        if total_batch % args.evaluate_steps == 0:
            ...
            # evaluate
            loss_dev, acc_top, f1_top, acc_sec, f1_sec, acc_conn, f1_conn = training.evaluate(args, model, dev_loader)
    ...
    lgg.info("Train time usage: {}".format(time_dif))
    #acc_top_test, f1_top_test, acc_sec_test, f1_sec_test, acc_conn_test, f1_conn_test \
    #    = training.test(golf_args, model, test_loader)
...
if __name__ == '__main__':
    ...
    train(
        da_config=da_config,
        idrr_model_list=[ldsgm_model, golf_model],  # pass both baseline models for co-training; first model is LDSGM
        labeled_data=[train_data, golf_train_dataset],
        unlabeled_dataset=unlabeled_data,
        path_train_losses=path_train_losses,
        dev_iter=[ldsgm_dev_iter, golf_dev_loader],
        test_iter=[ldsgm_test_iter, golf_test_loader],
        path_snapshot=path_snapshot,
        path_ann=path_ann,
        path_dev_pred=path_dev_pred,
        path_dev_gold=path_dev_pred,
