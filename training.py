...
import time
from golf_utils import get_time_dif
...
def evaluate(args, model, data_loader, test=False):
    ...
    if test:
        ...
        for i in range(cut_off):
            if gold_sense_top[i] == predict_sense_top[i] and gold_sense_sec[i] == predict_sense_sec[i] and \
                    gold_sense_conn[i] == predict_sense_conn[i]:
